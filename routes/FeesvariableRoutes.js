...
const FeesCalculationRouter = express.Router();

FeesCalculationRouter.post(
  "/CreateFeeVariable",
    verifyAdminToken,
  upload,
  async (req, res) => {
    try {
      ...
      let nextId = 1;
      const existingvariable = await FeesvariableScheme.find(
        {},
        { config_id: 1 },
        { sort: { config_id: 1 } }
      );
      ...
      const newvariableID = `F_${formattedCounter}`;


      const newVariable = new FeesvariableScheme({
        variable_name: variableName,
        config_id: newvariableID,
        test_total: "00",
      });
      const savedVariable = await newVariable.save();
      const allVariables = await FeesvariableScheme.find();
      res.status(201).json({
        success: true,
        data: allVariables,
...
FeesCalculationRouter.post(
  "/addDataToFeesVariable",
  verifyAdminToken,
  upload,
  async (req, res) => {
    try {
      const { variable_name, configId, material_items, test_total } = req.body;
      const data = await FeesvariableScheme.findOne({ config_id: configId });
...
FeesCalculationRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      let searchConfigId = await FeesvariableScheme.find({
        config_id: configId,
      });
...
FeesCalculationRouter.delete(
  "/DeleteFeesCalculation/:id",
  verifyAdminToken,
  async (req, res) => {
    const { id } = req.params;
    try {
      const result = await FeesvariableScheme.findByIdAndDelete(id);
      // res.send(result)
      if (result) {
        const allVariables = await FeesvariableScheme.find();
...
FeesCalculationRouter.get("/dropIndexes", async (req, res) => {
  FeesvariableScheme.collection.dropIndexes(function (err, result) {
    console.log("dropped successfully");
    res.end();
  });
});
FeesCalculationRouter.get(
  "/generateConfigId",
  // verifyAdminToken,
  async (req, res) => {
    let nextId = 1;
    const existingVariables = await FeesvariableScheme.find(
      {},
      { config_id: 1 },
      { sort: { config_id: 1 } }
    );
...
FeesCalculationRouter.get(
  ...
  async (req, res) => {
    try {
      const variables = await FeesvariableScheme.find().populate(
        "material_items"
      );
...
export default FeesCalculationRouter;
